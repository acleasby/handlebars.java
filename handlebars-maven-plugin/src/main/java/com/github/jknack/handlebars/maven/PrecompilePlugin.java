...
public class PrecompilePlugin extends HandlebarsPlugin {
    ...
    /**
     * If true, templates will be put into a flat namespace, otherwise the will be relative to
     * the template root.
     */
    @Parameter
    private boolean flatNamespace = false;
    ...
    protected void doExecute() throws Exception {
        ...
        try {
            ...
            for (File file : files) {
                ...
                String handlebarsName = templateName;
                if (flatNamespace) {
                    handlebarsName = file.getName().replace(suffix, "");
                    if (!handlebarsNames.contains(handlebarsName)) {
                        handlebarsNames.add(handlebarsName);
                    } else {
                        throw new IllegalArgumentException("Duplicate template name in handlebars"
                                + " namespace: " + handlebarsName);
                    }
                }
                ...
                Template template = handlebars
                        .compileInline("{{precompile \"" + templateName + "\"}}");
                Map<String, Object> hash = new HashMap<String, Object>();
                hash.put("wrapper", amd ? "amd" : "none");
                hash.put("templateName", handlebarsName);
                Options opts = new Options
                        .Builder(handlebars, PrecompileHelper.NAME, TagType.VAR, nullContext,
                        template)
                        .setHash(hash)
                        .build();
